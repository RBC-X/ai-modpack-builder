"""Inline SVG icons (lucide-style, stroke-based) rendered to QIcon/QPixmap.

Each entry is the SVG *inner* markup with stroke="currentColor" so the
renderer can substitute any color before rasterizing.
"""
...
ICONS = {
    ...
    "foldercheck": '<path d="M20 13a2 2 0 0 0 2-2V8a2 2 0 0 0-2-2h-7.9a2 2 0 0 1-1.69-.9L9.6 3.9A2 2 0 0 0 7.93 3H4a2 2 0 0 0-2 2v13a2 2 0 0 0 2 2h6"/><path d="m9 14 2 2 4-4"/>',
    ...
    "tree": '<path d="M12 22v-9"/><path d="M12 13 9.5 10"/><path d="M12 16l-4.5-4.5"/><path d="M12 19l-7-7"/><path d="M12 13l2.5-3"/><path d="M12 16l4.5-4.5"/><path d="M12 19l7-7"/>',
    "package": '<path d="m7.5 4.27 9 5.15"/><path d="M21 8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16Z"/><path d="m3.3 7 8.7 5 8.7-5"/><path d="M12 22V12"/>',
}


def pix(name: str, color: str = "#A7ADB4", size: int = 18) -> "QPixmap":
    # ``currentColor`` lives on the shared outer <svg>, not inside most icon
    # fragments. Replace after composing the complete SVG so every stroke
    # keeps the exact green / muted / warning color requested by the design.
    svg = (_S + ICONS.get(name, ICONS["info"]) + "</svg>").replace("currentColor", color)
...
def icon(name: str, color: str = "#A7ADB4", size: int = 18) -> QIcon:
    return QIcon(pix(name, color, size))
